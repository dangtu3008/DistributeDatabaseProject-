...
go
create trigger insertAuthor on AUTHOR_Genre after insert as
begin
	declare @count int;
	select @count = count(AuthorID) from AUTHOR_Genre where AuthorID IN (select AuthorID from inserted)
	if (@count > 1)
	begin
...
go
create trigger insertCustomer on CUSTOMERS_Address after insert as
begin
	declare @count int;
	select @count = count(CustomerID) from CUSTOMERS_Address where CustomerID IN (select CustomerID from inserted)
	if (@count > 1)
	begin
...
go
create trigger insertOrderHanoi on ORDERS_HanoiBefore2021 after insert as
begin
...
go
create trigger insertOrderHCM on ORDERS_HCMBefore2021 after insert as
begin
	declare @countS1 int, @countS2 int, @countS3 int, @countS3_2 int;
	select @countS1 = count(OrderID) from [DESKTOP-84SCD87\SITE1].BookManagement.dbo.ORDERS_HanoiLater2021 where OrderID in (select OrderID from inserted)
	select @countS2 = count(OrderID) from [DESKTOP-84SCD87\SITE2].BookManagement.dbo.ORDERS_HCMLater2021 where OrderID in (select OrderID from inserted)
...
go
create trigger insertBookStock on BOOKSTOCK_HCMNoMoreThan60000 after insert as
begin
	declare @countS1 int, @countS2 int, @countS3 int, @countS3_2 int;
...
go
create trigger deleteCustomer on CUSTOMERS_Address after delete as
begin
...
go
create trigger deleteAuthor on AUTHOR_Genre after delete as
begin
...
go
create trigger deleteBookstockHCM on BOOKSTOCK_HCMNoMoreThan60000 after delete as
begin
